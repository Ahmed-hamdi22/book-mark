...
websiteConainer=[];
  ...
  websiteConainer=JSON.parse(localStorage.getItem("websites"));
  displayBookmarks();
...
websiteConainer.push(website);
clearInput();
displayBookmarks();
localStorage.setItem("websites", JSON.stringify(websiteConainer));

console.log(websiteConainer)
    }
...
function displayBookmarks(){
    var displayText = '';
    for(var i = 0; i < websiteConainer.length; i++){
        displayText += `
        <tr>
            <td>${i+1}</td>
            <td>${websiteConainer[i].name}</td>
            <td> 
...
function deleteWebsite(index) {
   websiteConainer.splice(index, 1);
    localStorage.setItem('websites', JSON.stringify(websiteConainer));
    displayBookmarks();
...
// var popupBox = document.getElementById('Alert-container');
// var bookmarksArray = [];
